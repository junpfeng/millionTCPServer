...
public:
	...
	int WaitNetMsg() {
		...
		timeval t = {1,0};  // 非阻塞
		int ret = select(maxSock + 1, &fdRead, &fdWrite, &fdExp, &t);  // 设置最大阻塞事件1s
		if (ret < 0) {
			printf("select 出错\n");
			Close();    
			return ret;
		...
		for (int n = (int)_clients.size() - 1; n >= 0; --n) {
			...
			if (FD_ISSET(_clients[n]->getcSock(), &fdRead)) {
				...
				{
				// 客户端主动断开连接后，服务器需要关闭套接字，否则与客户端相连的TCP就会处于TIME_WAIT状态
					printf("sock=%d exit\n", _clients[n]->getcSock());
	...
	int RecvData(clientSocket * cSock) {
		...
		while (cSock->getLastPos() >= sizeof(DataHeader)) {
			...
			if (cSock->getLastPos() >= header->dataLength) {
				...
				// 这个地方，我将第二个参数写成了_RecvBuf + header->dataLength,结果浪费了我一天时间排查
